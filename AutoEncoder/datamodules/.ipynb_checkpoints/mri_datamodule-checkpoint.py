...
class MRIDataModule(pl.LightningDataModule):
    def __init__(self, root,
                 ...
                 transform=None, pin_memory=True):
        """
    ...
    def setup(self, stage=None):
        ...
        if stage == "fit" or stage is None:
            self.train_set = get_dataset(self.dataset_name, root=self.root, split="train", modalities=self.modalities, fixed_pair=self.fixed_pair, transform=self.transform)
            self.val_set   = get_dataset(self.dataset_name, root=self.root, split="val", modalities=self.modalities, fixed_pair=self.fixed_pair, transform=self.transform)

        if stage == "test" or stage is None:
            self.test_set  = get_dataset(self.dataset_name, root=self.root, split="test",  modalities=self.modalities, fixed_pair=self.fixed_pair, transform=self.transform)
